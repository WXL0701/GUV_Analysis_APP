...
[saveDir, saveName, ~] = fileparts(SavePath);
tempFrameDir = fullfile(saveDir, [saveName '_temp_frames']);
if exist(tempFrameDir, 'dir')
    try, rmdir(tempFrameDir, 's'); catch, end
end
mkdir(tempFrameDir);
...
for t = 1:numel(files)
    ...
    frame = getframe(hFig);
    imwrite(frame.cdata, fullfile(tempFrameDir, sprintf('%06d.jpg', t)), 'Quality', 90);
end

close(hFig);

% Use ffmpeg to generate video
cmd = sprintf('ffmpeg -y -r %d -i "%s/%%06d.jpg" -c:v libx264 -preset veryfast -crf 28 -pix_fmt yuv420p "%s"', ...
    Cfg.Debug.VideoFPS, tempFrameDir, SavePath);
[status, cmdout] = system(cmd);
if status ~= 0
    warning('ffmpeg failed: %s', cmdout);
else
    try, rmdir(tempFrameDir, 's'); catch, end
end
end
