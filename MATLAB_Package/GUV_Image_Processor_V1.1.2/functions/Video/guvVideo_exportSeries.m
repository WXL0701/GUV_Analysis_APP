...
for i = 1:numel(tasks)
    ...
    wantMp4 = strcmpi(fmt, 'mp4');
    if wantMp4 && ~mp4Enable
        warning('[%s%03d] Format=mp4 requested but Cfg.Video.Mp4.Enable=false, fallback to avi.', prefix, SeriesID);
        wantMp4 = false;
    end
    ...
    if wantMp4
        ...
        vw = localMakeVideoWriter(tmpPath, 'avi', fps, q);
...
text(ax, tx, ty, sprintf('%g um', lenUm), 'Color', col, 'FontSize', fs, 'FontWeight','bold', 'Interpreter','none');
end
