...
if isstruct(varargin{1})
    ...
    if nargin >= 2 && ~isempty(varargin{2})
        ...
        % 只在输出目录查找配置 JSON（避免意外读取脚本目录的“模板 JSON”）
        cfgPath = localFindConfigJson(OutRoot);
        if ~isempty(cfgPath)
...
% -------------------- 并行池（可选） --------------------
usePar = false;
if isfield(Cfg,'Video') && isfield(Cfg.Video,'ParallelEnable')
    usePar = logical(Cfg.Video.ParallelEnable);
elseif isfield(Cfg,'Parallel') && isfield(Cfg.Parallel,'Enable')
...
poolSize = [];
if isfield(Cfg,'Parallel') && isfield(Cfg.Parallel,'PoolSize')
