...
if nargin == 1 && isstruct(varargin{1})
    ...
    if nargin >= 3 && ~isempty(varargin{3})
        ...
        % 只在 OutputPath 下寻找配置 JSON（避免意外读取脚本目录的“模板 JSON”）
        cfgPath = localFindConfigJson(OutputPath);
        if ~isempty(cfgPath)
            cfgUser = guvPipeline_LoadConfig(cfgPath);
        else
            error(['未找到配置 JSON。\n' ...
                   '请将模板文件 "guvPipeline_config_template.json" 复制到输出目录并重命名为 "guvPipeline_config.json"，\n' ...
                   '或在调用时显式传入 jsonPath：GUV_Pipeline(InputPath, OutputPath, jsonPath)。']);
        end
    end
...
try
    AllXYResults = guvCompute_collectAllXYResults(Cfg.OutRoot, Cfg.SeriesPrefix, Cfg.Read.CNames);
...
function cfgPath = localFindConfigJson(OutputPath)
% 仅在输出目录查找配置 JSON（模板 JSON 不参与自动加载）
cands = { ...
