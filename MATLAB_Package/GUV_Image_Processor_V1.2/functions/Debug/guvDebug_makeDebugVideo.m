...
[SavePathResolved, profileName] = localSelectVideoWriter(SavePath);
v = VideoWriter(SavePathResolved, profileName);
v.Quality = 100;
...
function [savePathOut, profileName] = localSelectVideoWriter(savePathIn)
savePathOut = savePathIn;
profileName = 'MPEG-4';
try
...
if strcmpi(profileName, 'Motion JPEG AVI')
    [p, n] = fileparts(savePathIn);
    savePathOut = fullfile(p, [n '.avi']);
end
end
