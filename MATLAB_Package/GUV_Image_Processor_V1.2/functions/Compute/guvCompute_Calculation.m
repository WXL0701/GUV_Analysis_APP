...
%   AllResults : 结构体数组（索引=轨迹ID）
    ...
    for id_idx = 1:length(id_list)
        ...
        if any(is_valid)
            ...
            AllResults(id).Frames = frames(mask);
            
            % Centroids: 假设是 N x 2
            if size(centroids_track, 1) == num_files
                AllResults(id).Centroids = centroids_track(mask, :);
            elseif size(centroids_track, 2) == num_files
                AllResults(id).Centroids = centroids_track(:, mask)';
            else
                AllResults(id).Centroids = centroids_track; % 维度不匹配，暂保留
            end
            
            % MajorAxisLength
            if length(majorAxisLength) == num_files
                AllResults(id).MajorAxisLength = majorAxisLength(mask);
            else
                AllResults(id).MajorAxisLength = majorAxisLength;
            end
            
            AllResults(id).Volume_um3 = volumes_px(mask) * pixel_size^3;
            AllResults(id).Surface_um2 = surfaces_px(mask) * pixel_size^2;
            AllResults(id).Nu = nu_array(mask);
            AllResults(id).M_tilde = M_tilde_array(mask);
            AllResults(id).I_Gauss = I_K_array(mask);
            AllResults(id).Rb = rb_array(mask);
            AllResults(id).R_ve_um = R_ve_array(mask) * pixel_size;
            AllResults(id).StoredData = stored_data(mask);
            AllResults(id).SymmetryScore = symmetry_score_array(mask);
            AllResults(id).IsValid = is_valid(mask); % 应全为 true
            
            % 强度数据筛选
            AllResults(id).meanIntensities_bg_C01 = [];
            AllResults(id).meanIntensities_bg_C02 = [];
            AllResults(id).meanIntensities_inner_C01 = [];
            AllResults(id).meanIntensities_inner_C02 = [];
            AllResults(id).meanIntensities_mem_C01 = [];
            AllResults(id).meanIntensities_mem_C02 = [];
            ...
            for f = fieldList
                fn = f{1};
                val = intensityData.(fn);
                if ~isempty(val) && length(val) == num_files
                    AllResults(id).(fn) = val(mask);
                else
                    AllResults(id).(fn) = val;
                end
            end
            
        else
            % 如果没有任何有效帧，保留空数组
            AllResults(id).Frames = [];
            AllResults(id).Centroids = [];
            AllResults(id).MajorAxisLength = [];
            AllResults(id).Volume_um3 = [];
            AllResults(id).Surface_um2 = [];
            AllResults(id).Nu = [];
            AllResults(id).M_tilde = [];
            AllResults(id).I_Gauss = [];
            AllResults(id).Rb = [];
            AllResults(id).R_ve_um = [];
            AllResults(id).StoredData = [];
            AllResults(id).SymmetryScore = [];
            AllResults(id).IsValid = [];
