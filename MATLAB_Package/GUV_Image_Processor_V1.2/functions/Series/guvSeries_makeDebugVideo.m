...
ext = localSelectDebugVideoExt();
mp4Ref = fullfile(Paths.OutVideo, sprintf('%s_Debug_refC%02d.%s', Paths.SeriesName, refC, ext));
...
if ~isempty(otherC) && saveBoth
    mp4Oth = fullfile(Paths.OutVideo, sprintf('%s_Debug_othC%02d.%s', Paths.SeriesName, otherC, ext));
...
function ext = localSelectDebugVideoExt()
ext = 'mp4';
try
