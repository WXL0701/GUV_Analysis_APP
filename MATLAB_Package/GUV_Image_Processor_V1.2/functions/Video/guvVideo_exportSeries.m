...
% -------------------- 输出目录与 VideoWriter --------------------
% Unified output: .../output/final (sibling to XY folder if seriesFolder is inside RunID)
parentDir = fileparts(seriesFolder);
outDir = fullfile(parentDir, 'output', 'final');
...
fmtReq = fmt;
[fmt, videoProfileName] = localSelectVideoProfile(fmt);
fps = 10;
...
renderMode = 'array';
if isfield(Cfg.Video,'RenderMode') && ~isempty(Cfg.Video.RenderMode)
    renderMode = lower(char(Cfg.Video.RenderMode));
end

mp4Enable = false;
mp4Crf = 20;
mp4Preset = 'veryfast';
mp4DeleteSource = true;
if isfield(Cfg.Video,'Mp4') && isfield(Cfg.Video.Mp4,'Enable')
    mp4Enable = logical(Cfg.Video.Mp4.Enable);
end
if isfield(Cfg.Video,'Mp4') && isfield(Cfg.Video.Mp4,'CRF') && ~isempty(Cfg.Video.Mp4.CRF)
    mp4Crf = double(Cfg.Video.Mp4.CRF);
end
if isfield(Cfg.Video,'Mp4') && isfield(Cfg.Video.Mp4,'Preset') && ~isempty(Cfg.Video.Mp4.Preset)
    mp4Preset = char(Cfg.Video.Mp4.Preset);
end
if isfield(Cfg.Video,'Mp4') && isfield(Cfg.Video.Mp4,'DeleteSource') && ~isempty(Cfg.Video.Mp4.DeleteSource)
    mp4DeleteSource = logical(Cfg.Video.Mp4.DeleteSource);
end
...
for i = 1:numel(tasks)
    ...
    wantMp4 = strcmpi(fmtReq, 'mp4');
    if wantMp4 && ~mp4Enable
        warning('[%s%03d] Format=mp4 requested but Cfg.Video.Mp4.Enable=false, fallback to %s.', prefix, SeriesID, fmt);
        wantMp4 = false;
    end

    baseName = sprintf('%s%03d_%s', prefix, SeriesID, tk);
    if wantMp4
        tmpPath = fullfile(outDir, [baseName '.avi']);
        outPath = fullfile(outDir, [baseName '.mp4']);
        vw = localMakeVideoWriter(tmpPath, videoProfileName, fps, q);
        trans.(tk) = struct('tmp', tmpPath, 'out', outPath);
    else
        outName = sprintf('%s.%s', baseName, fmt);
        outPath = fullfile(outDir, outName);
        vw = localMakeVideoWriter(outPath, videoProfileName, fps, q);
        trans.(tk) = struct('tmp', outPath, 'out', outPath);
    end
    open(vw);
    writers.(tk) = vw;
...
useFigure = strcmpi(renderMode, 'figure');
if useFigure
    ...
    [sbRgb, sbMask] = localBuildScaleBarOverlay(W, H, Cfg);
    ...
    for t = 1:T
        ...
        for kk = 1:numel(taskList)
            ...
            frame = uint8(min(255, max(0, round(rgb * 255))));
            if ~isempty(sbMask)
                for c = 1:3
                    fc = frame(:,:,c);
                    oc = sbRgb(:,:,c);
...
for kk = 1:numel(taskList)
    tk = taskList{kk};
    if isfield(trans, tk)
        tmpPath = trans.(tk).tmp;
        outPath = trans.(tk).out;
        if ~strcmp(tmpPath, outPath) && mp4Enable
            [ok, msg] = localFfmpegAviToMp4(tmpPath, outPath, fps, mp4Crf, mp4Preset);
            if ~ok
                warning('[%s%03d] ffmpeg mp4 encode failed: %s', prefix, SeriesID, msg);
            elseif mp4DeleteSource
                try
...
function [fmtOut, profileName] = localSelectVideoProfile(fmtIn)
fmtIn = lower(string(fmtIn));
profiles = VideoWriter.getProfiles();
names = string({profiles.Name});
hasProfile = @(n) any(strcmpi(names, n));

switch fmtIn
    case "mp4"
        if hasProfile("MPEG-4")
            fmtOut = 'mp4';
            profileName = 'MPEG-4';
            return;
        end
        fmtOut = 'avi';
        if hasProfile("Motion JPEG AVI")
            profileName = 'Motion JPEG AVI';
        elseif hasProfile("Uncompressed AVI")
            profileName = 'Uncompressed AVI';
        else
            error('No supported VideoWriter profile found for AVI fallback.');
        end
    case "avi"
        fmtOut = 'avi';
        if hasProfile("Motion JPEG AVI")
        ...
        elseif hasProfile("Uncompressed AVI")
            ...
            error('No supported VideoWriter profile found for AVI.');
        end
    otherwise
        error('Unsupported video format: %s', char(fmtIn));
end
end

function v = localMakeVideoWriter(outPath, profileName, fps, q)
v = VideoWriter(outPath, profileName);
if isprop(v, 'Quality')
...
function [rgb, mask] = localBuildScaleBarOverlay(W, H, Cfg)
rgb = uint8([]);
mask = [];
...
set(ax,'YDir','reverse');
hold(ax,'on');
imshow(zeros(H,W,3,'uint8'), 'Parent', ax);
...
function [ok, msg] = localFfmpegAviToMp4(inAvi, outMp4, fps, crf, preset)
ok = false;
msg = '';
