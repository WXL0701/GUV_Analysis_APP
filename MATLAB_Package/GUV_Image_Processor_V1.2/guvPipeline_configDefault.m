...
% =========================================================================
% 重要原则（团队协作建议）：
%   1) 只在本文件改“开关/阈值”。算法实现细节在 Detect/Fuse/Track 内部函数。
...
%
% 使用（推荐 JSON 驱动）：
%   1) 复制模板 guvPipeline_config_template.json → 输出目录并重命名为 guvPipeline_config.json
%   2) 修改 JSON 后运行：
%      MasterTable = GUV_Pipeline(InputPath, OutputPath);
%
% 兼容旧版：
%   Cfg = guvPipeline_configDefault();  % 作为参数基线/注释参考
%   Cfg.ND2Path = '...'; Cfg.OutRoot = '...';
%   MasterTable = GUV_Pipeline(Cfg);

Cfg = struct();

%% ============================= 1) 路径与输出 =============================
Cfg.ND2Path = 'G:\人造细胞\HAO\20251224.nd2';
Cfg.OutRoot = 'G:\人造细胞\HAO\20251224_output1';
...
%% ============================= 3) 读取设置（Series / Z / 通道） ===========
Cfg.Read = struct();
Cfg.Read.SelectXYs = [8];     % 例如 [2 5 10]；空=[]表示全跑
...
Cfg.Fuse.Pair.MaxDist_um = 2;    % 质心配对距离门限（µm），典型 3~10
...
% --- 预处理/阈值 ---
opts.bin.sigma = 1.1;        % 高斯平滑 sigma（px）
opts.bin.sensitivity = 0.15;        % adaptive threshold 灵敏度（0~1，越大越容易判为前景）
opts.bin.adapt_sensitivity = opts.bin.sensitivity; % backward compat (old key)
opts.bin.minHoleArea = 15;   % 填洞最小面积（px^2）
...
Cfg.Post.Watershed.MaxChild = 5;

%% ============================= 7) 追踪（Track） ==========================
Cfg.Track = struct();
Cfg.Track.DistGate_um = 5;      % 帧间关联距离门限（µm）
Cfg.Track.MaxGap = 1;           % 最大断帧数
Cfg.Track.MinLen = 10;           % 最短轨迹长度（过滤噪声）
Cfg.Track.StartFrame = [];       % 可选：追踪起始帧，1-based；[] 表示不限制
Cfg.Track.EndFrame = [];         % 可选：追踪结束帧，1-based；[] 表示不限制
Cfg.Track.FrameRange = [];       % 可选：[start end]，优先于 StartFrame/EndFrame
...
Cfg.Track.Opts.EstimateGlobalDrift = false;  % 全局漂移估计（整体漂移明显时强烈建议 true）
...
Cfg.Output.SaveCSV           = false;

%% ============================= 9) 调试（Debug） ==========================
Cfg.Debug = struct();
Cfg.Debug.Enable       = true;  % 是否允许输出 debug（总开关）
Cfg.Debug.SingleXYOnly = false;  % 批量跑时只对"单XY"输出 PNG/视频（避免爆炸）
Cfg.Debug.MaxFrames    = [];    % 限制最大帧数（空=[]表示全帧）
Cfg.Debug.SaveFramePNG = false;  % 输出逐帧PNG（单XY调试）
...
%% ============================= 11) 追踪后计算（可选） ==================
% 来自你提供的 "GUV-Image-Processor-main" 的 Calculation 脚本。
% 注意：结果人工检查（VisualizeSingleID）已从主流程摘出，放入独立的
% VisualizeSingleID_Pipeline.zip，避免在批量流程中自动生成大量图像。
Cfg.Compute = struct();
Cfg.Compute.Enable = false;    % true=在每个 Series 跑完追踪后，额外计算 AllResults
...
Cfg.Video.ParallelEnable = false;
Cfg.Video.RenderMode = 'array';
...
Cfg.Video.Mp4 = struct();
Cfg.Video.Mp4.Enable = true;
Cfg.Video.Mp4.CRF = 20;
Cfg.Video.Mp4.Preset = 'veryfast';
Cfg.Video.Mp4.DeleteSource = true;

% 导出任务：支持单通道 + Merge（双通道伪彩叠加）
% 可选值：{'C1','C2','Merge'}，其中 C1/C2 对应 Cfg.Read.CList(1/2)
Cfg.Video.Tasks = {'C1','C2','Merge'};
...
Cfg.Video.ScaleBar.Position     = [1500 2000]; % [X Y] 像素坐标（建议为标尺左端的1底边"点：[x_left, y_bottom]）。[] 表示自动放在右下角
Cfg.Video.ScaleBar.FontSize     = 48;
Cfg.Video.ScaleBar.Color        = [1 1 1];

% 时间戳
Cfg.Video.TimeStamp = struct();
Cfg.Video.TimeStamp.Enable     = false;
